...
class InteractionVideoSystem(L.LightningModule):
    ...
    def forward(self, model_input, model_input2, first_frames, prompt_embeds):
        ...
        if self.hparams.use_DiffSynth:
            ...
            # breakpoint()
            #
            kkk = (timestep.view(batch_size, 1, 1, 1) * mask2[0][:, 0, :, ::2, ::2]).flatten(1)
            v_pred = self.transformer(
                hidden_states=latent_noisy, # B, C, F, H, W
                encoder_hidden_states=prompt_embeds,
                # 这里timestep要double, 后面是条件，所以t设置为0.
                # 1转3需要加上角色参考图
                timestep=torch.concat([kkk, torch.zeros_like(kkk), torch.zeros_like(mask2[0][:, 0, 0, ::2, ::2].flatten(1))], dim=-1) if self.hparams.dataset.is_one2three else torch.concat([kkk, torch.zeros_like(kkk)], dim=-1),
                return_dict=False,
    ...
    def on_predict_epoch_start(self):
        ...
        self.pred_path = os.path.join(
            '/raid/workspace1/lc/HumanAndRobot/Output_Inference/HumanAndRobot_High')
...
def main(opt):
    ...
    trainer.fit(system,
                ...
                ckpt_path=opt.ckpt_path, # 断点恢复，单纯导入模型权重可以system.load_state_dict()
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ckpt_path", type=str, default="", help="path to a .ckpt to resume from")
    # ----------------------------------------------------------------------
